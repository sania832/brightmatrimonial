...
{   
	...
	*/
	...
	public function create(Request $request){
		...
		}elseif(empty(session()->get('shop_id'))){
			$this->ajaxError([], 'Invalid Shop');
		}
		...
		try{
			...
			if($cartData->count() > 0){
				...
				if($insert){
					...
					DB::commit();
					$this->sendResponse($insert, trans('order.order_create_success'));
				}
			}
			
			$this->ajaxError([], trans('order.order_create_failed'));
		} catch (Exception $e) {
			DB::rollback();
